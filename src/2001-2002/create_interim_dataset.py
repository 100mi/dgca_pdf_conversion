...
def save_csv(df, file_path, table_name):
    ...
    # /home/somi/DS/dgca_pdf_conversion/data/raw/yearly/2001-2002/air-transport-statistics-for-the-year-2001-2002/air-india/aircraft-by-type-and-size-owned-by-air-india-for-last-five-years/2-05.PDF
    split_path = file_path.split('/2001-2002/')
    output_path = Path(split_path[0].replace('/raw/','/interim/')+'/'+"2001-2002"+'/'+'/'.join(split_path[-1].split('/')[1:-1]))
    ...
    for each_path in get_files():
        for dataset in read_pdf(each_path):
            ...
            try :
                ...
                print("== == == == ==", each_path.split('2002/')[-1], ": ", e)
                with open("./err.txt", "a") as error:
                    error.write(str(each_path.split('2002/')[-1]) + "\n")
