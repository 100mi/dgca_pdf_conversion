...
def save_csv(df, file_path, table_name):
    ...
    output_path = Path(split_path[0]+'/'+"interim/yearly/2004-2005"+'/'+ mod_airline_name + '/' + mod_table_name)
    ...
    for each_path in get_files():
        for dataset in read_pdf(each_path):
            ...
            try :
                ...
                print("== == == == ==", each_path.split('2005/')[-1], ": ", e)
                with open("./err.txt", "a") as error:
                    error.write(str(each_path.split('2005/')[-1]) + "\n")
