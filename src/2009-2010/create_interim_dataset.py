...
def get_table_name(df):
    ...
    title_val = [val for val in trial_val if not re.search(title_pattern,val) and len(val) > 30] 
    if len(title_val) > 0:
        return title_val[0]
    else :
        return trial_val[2]
...
def save_csv(df, file_path, table_name):
    ...
    split_path = file_path.split('/2009-2010/')
    output_path = Path(split_path[0].replace('/raw/','/interim/')+'/'+"2009-2010"+'/'+ mod_airline_name + '/' + mod_table_name)
    ...
    for each_path in get_files():
        ...
        for dataset in read_pdf(each_path):
            ...
            try :
                ...
                print("== == == == ==", each_path.split('2010/')[-1], ": ", e)
                with open("./err.txt", "a") as error:
                    error.write(str(each_path.split('2010/')[-1]) + "\n")
