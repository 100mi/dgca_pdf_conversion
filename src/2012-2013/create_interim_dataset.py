...
def save_csv(df, file_path, table_name):
    ...
    split_path = file_path.split('/2012-2013/')
    mod_table_name = text_modification(table_name)
    output_path = Path(split_path[0].replace('/raw/','/interim/')+'/'+"2012-2013"+'/'+'/'.join(split_path[-1].split('/')[1:-2])+'/'+mod_table_name)
        ...
        for dataset in read_pdf(each_path):
            ...
            try :
                ...
                with open("./err.txt", "a") as error:
                    error.write(str(file_path.split('2013/')[-1]) + "\n")
